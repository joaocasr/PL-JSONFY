...
class converter:
    ...
    def toJson(self):
        ...
        if(lista!=None):
            ...
            tom = re.sub(r'[a-zA-Z]+\w\s?=\s?\[\n?(\"\w+\",?\n?)*\n?\]',onelinelist,self.toml)

        if(lista==None): tom=self.toml
        ...
        for p in self.allgroups:
            lista = p.split(".")
            if(len(lista)>1):
                nested=getNestedDict(lista,convert_dict(self.allgroups[p].split("\n")))
                dicjoin(dic,nested)
            else:
                dic.update({lista[0]:convert_dict(self.allgroups[lista[0]].split("\n"))})
